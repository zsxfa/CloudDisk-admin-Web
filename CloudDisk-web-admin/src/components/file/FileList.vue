...
// import fileList from '@/store/module/fileList'
	...
	methods: {
		...
		showFileRecovery() {
			getRecoveryFile().then((res) => {
				if (res.code == 0) {
					this.fileList = res.data.recoveryFileList
					this.pageData.total = res.data.total
					this.loading = false
				} else {
					this.$message.error(res.message)
		...
		getSearchFileList(fileName) {
			...
			searchFile({
				...
				this.loading = false
				if (res.code == 0) {
					// this.fileList = res.data.map.map((item) => item.list)
					this.fileList = res.data.map.list
					this.pageData.total = res.data.map.total
					console.log('pageData.total: ' + this.pageData.total)
				} else {
					this.$message.error(res.message)
				}
			})
