...
export default {
	data() {
		return {
			// 上传组件配置项
			options: {
				target: `${this.$config.baseContext}/api/core/hdfs/uploadfile`, // 上传文件-目标 URL
				chunkSize: 1024 * 1024 * 1024 * 10, //  每个分片的大小
				...
				checkChunkUploadedByResponse: function (chunk, message) {
					...
					if (objMessage.code == 0) {
						...
						if (data.uploadFileVo.skipUpload) {
							// 分片已存在于服务器中
							return true
						}
						console.log('objMessage.message: ' + objMessage.message)
						// return (
						//   // (data.uploadFileVo.uploaded || []).indexOf(chunk.offset + 1) >= 0
						//   // data.uploadFileVo.uploaded
						// )
					} else {
						return true
					}
	...
	methods: {
		...
		handleFileSuccess(rootFile, file, response) {
			...
			if (result.code == 0) {
				console.log('Box.vue 中的299行，成功上传了')
		...
		computeMD5(file) {
			...
			let currentChunk = 0
			const chunkSize = 1 * 1024 * 1024 * 1024 * 10
